...
while (cin >> n) {
    ll m; cin >> m;
        ...
        vector<bool> isAwake(26, false);
        foreach(c, awake) {
            isAwake[c - 'A'] = true;
            present[c - 'A'] = true;
        }

        ll awakeCount = 3, years = 0;
        while (1) {
            vector<int> toWake;
            up(0, 26, 1) {
                if (!present[i] || isAwake[i]) continue;
                ll cnt = 0;
                foreach(nb, g[i]) if (isAwake[nb]) cnt++;
                if (cnt >= 3) toWake.push_back(i);
            }
            if (toWake.empty()) break;
            foreach(x, toWake) {
                isAwake[x] = 1;
                awakeCount++;
            }
            years++;
            if (awakeCount == n) break;
        }
